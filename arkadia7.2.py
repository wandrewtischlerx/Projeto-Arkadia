...
# Cache e Fila para Mídias (Imagens/Vídeos)
_media_cache = {}
    ...
    while True:
        ...
        try:
            ...
            for grupo_tuple in grupos:
                ...
                # Enfileira prefetch de mídia sempre
                if link0:
                    with _geo_refine_lock:
...
def _worker_media_prefetch():
    """Thread que processa a fila de scraping em segundo plano."""
    while True:
        item_url = None
        with _geo_refine_lock:
            if _media_queue:
        ...
        try:
            # Simula a chamada da API interna para popular o cache
...
def api_scrape():
    ...
    url = flask_req.args.get("url", "").strip().split('?')[0] # Limpa trackers
    
    # Verifica se já está no cache de prefetch
    with _media_lock:
        if url in _media_cache:
            return jsonify(_media_cache[url])
    ...
    def scrape_one(u):
        try:
            ...
            def _add_img(src):
                ...
                if not src or not src.startswith("http") or _is_logo_url(src) or src in images:
                    return False
                # Filtra apenas links que terminam com extensões de imagem conhecidas
                exts = ('.jpg', '.jpeg', '.png', '.webp', '.gif', '.bmp', '.jfif', '.avif')
                if any(src.lower().split('?')[0].endswith(ext) for ext in exts):
                    images.append(src)
                    return True
                return False
            ...
            # ── 6. Varredura por links diretos (tags <a>) ────────────────────────
