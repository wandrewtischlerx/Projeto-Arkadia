...
def api_scrape():
    ...
    def scrape_one(u):
        try:
            ...
            r = _http.get(u_real, timeout=9, headers={
                "User-Agent": (
                    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                    "AppleWebKit/537.36 (KHTML, like Gecko) "
                    "Chrome/124.0.0.0 Safari/537.36"
            ...
            # OG / Twitter meta tags — prioridade máxima, geralmente é a imagem principal
            for attr, val in [
                ...
                if tag:
                    src = tag.get("content", "")
                    if src.startswith("http") and not _is_logo_url(src) and src not in images:
                        images.append(src)
            ...
            for sel in selectors:
                for img in soup.select(sel)[:10]:
                    ...
                    if not src or not src.startswith("http"):
                        continue
                    if _is_logo_url(src) or src in images:
                        continue
                    # Filtra ícones/logos pequenos por dimensão declarada
                    w = img.get("width", "")
                    h = img.get("height", "")
                    try:
                        if int(w) < 120 or int(h) < 80:
                            continue
                    except Exception:
                        pass
                    images.append(src)
